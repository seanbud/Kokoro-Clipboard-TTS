...
request_counter = 0
SAMPLERATE = 24000
# Leading-silence trim: samples below this fraction of peak amplitude are
# considered silent at the start of a chunk (only applied to chunk 0).
SILENCE_THRESHOLD_RATIO = 0.01
MIN_SILENCE_THRESHOLD = 1e-4
# Prepended silence on the first chunk to absorb audio-device startup latency.
STARTUP_SILENCE_PAD_S = 0.04   # 40 ms
# Polling interval when waiting for the callback-based OutputStream to finish.
STREAM_POLL_INTERVAL_S = 0.01  # 10 ms
...
def play_audio_with_pause_support(audio_data):
    """
    Play mono float32 audio via a callback-based OutputStream.

    - Pauses (outputs silence) when pause_event is set, without losing position.
      Position resumes from exactly where it was when the user resumes.
    - Stops immediately when stop_event is set.
    - Blocks the calling thread until playback is complete or stopped.

    Using a callback-based stream instead of sd.play()+sd.wait() means the
    audio device stays warm between chunks (no per-call device-open overhead),
    which eliminates the startup-latency clipping audible at higher TTS speeds.
    """
    pos = [0]
    ...
    def callback(outdata, frames, time_info, status):
        ...
        if pause_event.is_set():
            outdata[:] = 0  # Output silence; do NOT advance position
            return
        ...
        outdata[:n, 0] = audio_data[pos[0]:pos[0] + n]
        pos[0] += n
        if n < frames:
            outdata[n:] = 0
            raise sd.CallbackStop()
...
def tts():
    ...
    pause_event.clear()  # clear pause first so the callback exits via stop, not silence loop
    stop_event.set()
    sd.stop()
    ...
    # Unbounded queue so the generator thread never blocks on put() even if
    # stop_event is set while the queue is full (avoids a deadlock).
    ...
    def generator_worker():
        ...
        try:
            ...
            # Always signal end-of-stream so the playback worker can exit cleanly
    ...
    def playback_worker():
        ...
        try:
            while not stop_event.is_set():
                ...
                # For the first chunk: trim any leading silence generated by the
                # TTS model, then prepend a tiny startup pad to absorb audio-device
                # initialisation latency.  At higher speeds (e.g. 1.5x) the first
                # chunk is shorter, so even small fixed latencies clip a noticeable
                # fraction of the speech without this compensation.
                if i == 0 and len(played_audio) > 0:
                    max_amp = float(np.max(np.abs(played_audio)))
                    threshold = max(max_amp * SILENCE_THRESHOLD_RATIO, MIN_SILENCE_THRESHOLD)
                    non_silent = np.where(np.abs(played_audio) > threshold)[0]
                    if len(non_silent) > 0:
                        played_audio = played_audio[non_silent[0]:]
                    # Prepend silence to absorb audio-device startup latency
                    silence_pad = np.zeros(int(SAMPLERATE * STARTUP_SILENCE_PAD_S), dtype=np.float32)
                    played_audio = np.concatenate([silence_pad, played_audio])
                ...
                play_audio_with_pause_support(played_audio)
...
def stop():
    ...
    pause_event.clear()  # clear pause so the callback exits via stop_event, not silence loop
    stop_event.set()
    sd.stop()
